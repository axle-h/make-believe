...
## 2026-09-02 — the TV is all game (at Alex's request) — Claude (Opus 5)
- Did: the world now fills the whole TV. The "MAKE believe" banner is gone, and with it the join URL and the four-letter code in text; the QR code floats over the top-right corner of the game with a small margin, a light border and a shadow, and the phase/status line sits in a quiet pill at the bottom-left. The code was always inside the QR code's URL as `?room=` and that is now the only place it lives (D-025).
- And on the phone: there is no code field any more. A page opened from the QR link (or holding a code from last time) asks only for a name; a page opened at the bare URL shows a scan screen — "Scan the QR code on the TV to join" — and every way of failing to get in, including a stale code, lands back there instead of on a form.
- The host's code moved to the test seam as `window.__game.roomCode`, because the e2e suite used to read it out of `#room-code`. New e2e test: a phone at the bare URL is sent to scan, and following the QR link gets it in.
- Verified: `pnpm typecheck`, `pnpm lint`, `pnpm test` (146 tests, 15 files), `pnpm build`, `pnpm test:e2e` (5 tests). Screenshotted the built app at 1920x1080 and on a Pixel-7-shaped phone to check the QR panel and the scan screen by eye.
- Note for Alex: nothing on the TV spells the code out now, so a laptop-on-the-sofa player has to be sent the link rather than read the code. Say the word if that bites and the TV can reveal it on a keypress.
- Next: phase 9, the PWA.

## 2026-09-02 — no rounds: one continuous session (at Alex's request) — Claude (Opus 5)
- Did: ripped the phase machine out, root and branch (D-026). Alex set out how the blob game actually works — join by QR, then drive, talk, draw and rename whenever you like — and that the TV takes no input at all.
- Model: `GameState.phase` gone, `phases.ts` and its transition table deleted, `applyMessage` no longer asks what the world is doing before accepting text or a drawing. `IgnoredReason` is down to `unknown-player`.
- Protocol: `{ type: 'phase', … }` replaced by `{ type: 'waiting' }`, sent by the relay only and meaning "no TV for you: wait and keep knocking". `assigned` is now the phone's cue that it is in, since it only ever answers a hello. `HostOutboundMessage` is a single message type now rather than a union.
- TV: no `keydown` handler, no shortcut footer, no title, nothing to press. What is left is the world, the QR code, and a connection status that is invisible unless something is wrong.
- Phone: one controller screen — name, joystick, and a Say / Draw / Name bar. Those three are overlays over the joystick, one tap away and one tap back; opening one stops the blob so a thumb leaving the pad cannot leave it running. Renaming needs no new message: saying hello again with a new name is a rename, and `join` already keeps a known `playerId`'s blob. Sending a drawing puts the joystick back up — the blob changing on the TV is the better receipt.
- Verified: `pnpm typecheck`, `pnpm lint`, `pnpm test` (138 tests, 14 files), `pnpm build`, `pnpm test:e2e` (6 tests). New e2e test drives, renames mid-game, redraws twice (`skin-…-1` then `-2`, both as far as the sprite) and drives again afterwards. Screenshotted the controller and each of the three sheets on a Pixel-7-shaped phone.
- Also updated `CLAUDE.md`: the feature list now says one continuous session with no rounds and no host input, the protocol block shows `waiting`, and the testing notes no longer ask for phase-transition tests.
- Next: phase 9, the PWA — and milestone 11, actual game ideas, is the place rounds would come back if they ever do.
