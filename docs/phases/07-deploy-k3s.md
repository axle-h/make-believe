...
- [x] Image publish, documented in `k8s/README.md`. **Changed by Alex on 2026-09-02: built by CI, not by hand.** `.github/workflows/container.yml` builds the image, starts it the way the Deployment does (read-only, non-root, no capabilities), proves it serves both pages and relays a join, and pushes `ghcr.io/axle-h/make-believe:latest` and `:<sha>` on main. The package came out public on its first push and pulls anonymously, so no visibility click was needed (D-023).
  ```sh
...
- [x] Apply to the cluster. Namespace `make-believe` created, deployment and service applied, pod Running and Ready on the first try with no restarts. `kubectl create namespace make-believe`, `kubectl -n make-believe apply -f ./k8s/make-believe`, wait for the pod to be Running and Ready.
- [x] Verify in-cluster. Skipped the port-forward in favour of the real thing: phase 8 went in immediately afterwards, so it was verified through the public URL instead. `kubectl -n make-believe port-forward svc/make-believe 3000:80` and open `http://localhost:3000/host/`. Note that WebSockets work through a port-forward, so a phone-less check of host + two browser players is possible from the laptop.
...
- **State:** done and running. `kubectl -n make-believe get all` shows one pod, Ready, and the service on 80. Superseded detail: the manifests were written before the image existed; the image is now CI-built (see the task above).
- **Previously:** manifests written and dry-run clean; nothing applied and nothing pushed. `k8s/make-believe/deployment.yml` (replicas 1, Recreate, `PORT=3000`, container port 3000 named `http`, startup and liveness probes on `/healthz`, non-root uid 1000, read-only root filesystem, all capabilities dropped, 50m/64Mi requests and a 256Mi memory limit) and `k8s/make-believe/service.yml` (ClusterIP 80 → `http`). `k8s/README.md` covers the apply, the image publish and the GHCR-private trap. The container was rebuilt with the phase 3 to 6 code and run with `--read-only --cap-drop ALL --user 1000:1000`, serving `/healthz`, `/host/` and `/`.
- **Next step:** nothing here. Phase 8 is done too; phase 9 (the PWA) is next.
- **Known issues:** none. `readOnlyRootFilesystem: true` is the one line that would fail the pod if anything ever starts writing to disk; CI now runs the image under exactly that constraint before publishing it, so it cannot first be discovered in the cluster.
