...
- **The TV keeps itself on the current build too**, the same way the phones do — `/version` on every socket and every couple of minutes besides, because a stick behind a television is switched on and left, and a TV a fortnight behind its phones is the one staleness nobody in the room can see. But the safe moment is far narrower than a phone's: the host holds *all* the game state, so a reload destroys every blob, every name and every picture a child drew, and the relay mints a fresh session behind it so all the phones come back as new players. It therefore reloads **only into a world with no blobs in it at all** — not between tasks, not during an interlude, and never for a blob that is merely away, whose child is coming back. An update can stay pending all evening, and that is correct. The rule is `src/host/updates.ts` and that is where it is unit-tested.
...
- Staleness of the *build* is decided by one thing: `/version` against the page's own `__BUILD_VERSION__`, checked on every connect and whenever a new worker takes over. A mismatch reloads the phone — but only on the waiting screen, never mid-joystick (`src/player/updates.ts`, which is where that rule is unit-tested). **Both pages ask the same question and `src/lib/version.ts` is what asks it** — including the guard that an answer which is not a version is not an answer, because the dev server serves the whole index page at `/version` and a page that took that for a build would reload itself for ever.
